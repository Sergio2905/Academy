...
showContactsFormBlocks();


function nextStep(btn) {
    var data = btn.data("target");
    if (data) {
        $('.form__step').removeClass('active');
        $(`#${data}`).addClass('active');
        $([document.documentElement, document.body]).animate({
            scrollTop: $('.form__body').offset().top
        }, 500);
    }
}

function showContactsFormBlocks() {
    $('.block-form__body').not('.notice__form .block-form__body').removeAttr('hidden');
}
...
$(document).on('click', (e) => {
    ...
    var formBtn = $(e.target).closest('.form__btn');
    if (formBtn.length) {
        nextStep(formBtn);
    }
});
